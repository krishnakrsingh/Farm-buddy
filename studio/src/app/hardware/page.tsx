...
import { Cpu, Wifi, WifiOff, Heart, Thermometer, Activity, Radio, ArrowLeft, Zap, Eye } from "lucide-react";
...
export default function HardwarePage() {
    ...
    const { isHardwareOnline, isFirebaseConnected } = useHerdState();
    const { liveData, lastSeen, connectionLatency, hardwareHistory } = useLiveSensorData("014");
        ...
        <div className="min-h-screen bg-[#DBEDD9] text-[#1B4332] pb-32 relative font-sans overflow-x-hidden selection:bg-[#B7D8C6]">
            <div className="max-w-md mx-auto relative pt-8 px-5 space-y-5 z-10 pb-10">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-black text-[#113A28] leading-none flex items-center gap-2">
                                <Cpu className="text-[#184F35] w-5 h-5" /> Hardware Status
                            </h1>
                            <p className="text-[10px] font-bold text-[#6C8576] mt-0.5">
                                ESP32-C3 Sensor Node — Cow #014
                            </p>
                        </div>
                    </div>
                ...
                <motion.div
                    ...
                    className={cn(
                        "rounded-[28px] p-4 border shadow-md transition-all",
                        isHardwareOnline
                ...
                >
                    <div className="flex items-center gap-3">
                        <div className={cn(
                            ...
                            isHardwareOnline ? "bg-emerald-800/60" : !isFirebaseConnected ? "bg-amber-700/60" : "bg-zinc-600"
                        )}>
                            {isHardwareOnline ? (
                        ...
                        <div>
                            ...
                            <p className="text-[11px] font-medium opacity-80 mt-1 leading-snug">
                                {isHardwareOnline
                                    ? `Device: ${liveData?.deviceId} • RSSI: ${liveData?.rssi} dBm • Latency: ${connectionLatency}ms`
                                    : !isFirebaseConnected
                                        ? "Add NEXT_PUBLIC_FIREBASE_DATABASE_URL to your .env.local file"
                                        : "Power on your ESP32-C3 sensor node to begin streaming"}
                            </p>
                        </div>
                    </div>
                    ...
                    {isHardwareOnline && liveData && (
                        <div className="mt-3 pt-3 border-t border-white/20 grid grid-cols-3 gap-2 text-center">
                            ...
                            <div>
                                <div className="text-[9px] font-black uppercase tracking-wider opacity-70">Last Seen</div>
                                <div className="text-[14px] font-black">
                                    {lastSeen ? `${Math.floor((Date.now() - lastSeen.getTime()) / 1000)}s` : "—"}
                                </div>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] font-bold text-[#6C8576]">Heart Rate + Pulse Oximeter</p>
                                </div>
                            </div>
                            <span className={cn(
                                "text-[9px] font-black px-2 py-0.5 rounded-full uppercase",
                                liveData?.sensors?.max30102
                                    ? "bg-emerald-100 text-emerald-700"
                                    : isHardwareOnline
                                        ? "bg-red-100 text-red-700"
                                        : "bg-zinc-100 text-zinc-500"
                            )}>
                                {liveData?.sensors?.max30102 ? "Active" : isHardwareOnline ? "Error" : "—"}
                            </span>
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                            <div className="bg-[#F8FBF8] rounded-[16px] p-2.5 border border-[#E9F4EC]">
                                ...
                                <div className="text-[22px] font-black text-[#113A28] leading-none mt-1">
                                    {isHardwareOnline ? `${liveData?.hr || "—"}` : "—"}
                                    <span className="text-[11px] font-bold text-[#6C8576] ml-1">bpm</span>
                                </div>
                            </div>
                            <div className="bg-[#F8FBF8] rounded-[16px] p-2.5 border border-[#E9F4EC]">
                                <div className="text-[9px] font-black text-[#6C8576] uppercase tracking-wider">SpO2</div>
                                <div className="text-[22px] font-black text-[#113A28] leading-none mt-1">
                                    {isHardwareOnline ? `${liveData?.spo2?.toFixed(1) || "—"}` : "—"}
                ...
                {chartData.length > 2 && (
                    ...
                    >
                        <h2 className="text-[14px] font-black uppercase tracking-wider text-[#113A28] px-1 flex items-center gap-2">
                            <Eye size={16} /> Live Sensor Stream
                        </h2>
                        ...
                        <div className="bg-white rounded-[24px] p-4 shadow-[0_8px_24px_rgba(0,0,0,0.04)] border border-white">
                            <div className="flex justify-between items-center mb-2">
                                <span className="text-[11px] font-black text-[#113A28]">Activity Level (%)</span>
                                <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full flex items-center gap-1">
                                    <Zap size={10} /> Live
                                </span>
                            </div>
                            <div className="h-[100px] w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    <LineChart data={chartData}>
                                        <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#E9F4EC" />
                                        <XAxis dataKey="time" tick={{ fontSize: 8, fill: "#8DA697" }} axisLine={false} tickLine={false} />
                                        <YAxis domain={[0, 100]} tick={{ fontSize: 8, fill: "#8DA697" }} axisLine={false} tickLine={false} width={30} />
                                        <Line type="monotone" dataKey="activity" stroke="#2196F3" strokeWidth={2} dot={false} isAnimationActive={false} />
                                    </LineChart>
                                </ResponsiveContainer>
                            </div>
                        </div>
                ...
                {/* Wiring Reference (visible when offline) */}
                {!isHardwareOnline && (
                    <motion.div
                        initial={{ opacity: 0, y: 15 }}
                        animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                        <h2 className="text-[14px] font-black uppercase tracking-wider text-[#113A28] mb-3 flex items-center gap-2">
                            <Cpu size={16} /> Quick Wiring Reference
                        </h2>
                        <div className="bg-[#F8FBF8] rounded-[18px] p-3 border border-[#E9F4EC] font-mono text-[11px] text-[#113A28] leading-relaxed space-y-1">
                            <div className="flex justify-between"><span className="text-[#6C8576]">GPIO 8 (SDA)</span><span>→ MAX30102 + MPU6500 SDA</span></div>
                            <div className="flex justify-between"><span className="text-[#6C8576]">GPIO 9 (SCL)</span><span>→ MAX30102 + MPU6500 SCL</span></div>
                            <div className="flex justify-between"><span className="text-[#6C8576]">GPIO 4</span><span>→ DS18B20 DATA + 4.7kΩ↑</span></div>
                            <div className="flex justify-between"><span className="text-[#6C8576]">3.3V</span><span>→ All sensor VCC</span></div>
                            <div className="flex justify-between"><span className="text-[#6C8576]">GND</span><span>→ All sensor GND</span></div>
                        </div>
                        <p className="text-[10px] font-medium text-[#6C8576] mt-2 leading-relaxed">
                            Upload the firmware from <code className="bg-[#F4F9F4] px-1 py-0.5 rounded text-[#184F35] font-bold">hardware/esp32_sensor_node/</code> to your ESP32-C3. Data will stream here automatically.
                        </p>
                    </motion.div>
                )}
            </div>

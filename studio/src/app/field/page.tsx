...
export default function FieldPage() {
    ...
    const FIELD_ZONES = [
        ...
        { id: 2, node: "N-02", status: "critical", moisture: "15%", temp: "30°C", issue: t("critical_dryness"), details: t("critical_dryness_details") },
    ];
    ...
    return (
        <div className="min-h-screen bg-[#DBEDD9] text-[#1B4332] pb-32 relative font-sans overflow-x-hidden selection:bg-[#B7D8C6]">
            ...
            <div className="max-w-md mx-auto relative pt-10 px-5 space-y-7 z-10 pb-10">
                ...
                <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 }}>
                    <div className="bg-white rounded-[32px] p-4 shadow-[0_12px_32px_rgba(0,0,0,0.05)] border border-white">
                        <div className="flex justify-between items-center mb-4 px-1 pt-1">
                            <div>
                                <h3 className="text-[18px] font-extrabold text-[#113A28] leading-snug">{t("sector_a")}</h3>
                                <div className="bg-[#FFF8DF] px-2 py-1 rounded-[10px] flex items-center gap-1 mt-1 w-max">
                                    <span className="text-[#E7A600] text-[11px] font-black uppercase tracking-wider">🟡 {t("moderate_risk")}</span>
                                </div>
                            </div>
                            <div className="flex bg-[#F4F9F4] p-1 rounded-xl border border-[#E9F4EC]">
                                <button className="bg-white text-[#184F35] text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg shadow-sm">{t("live")}</button>
                                <button className="text-[#8DA697] text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg">24H</button>
                            </div>
                        </div>

                        {/* Map */}
                        <div className={cn("relative h-[240px] w-full rounded-[24px] overflow-hidden border-[2px] border-[#E9F4EC] shadow-sm mb-4 transition-all duration-700 ease-in-out",
                            selectedNode ? "scale-[1.02] shadow-inner" : "scale-100"
                        )}>
                            <MapComponent selectedNode={selectedNode} setSelectedNode={setSelectedNode} />
                            {selectedNode && (
                                <div className="absolute inset-0 bg-black/30 z-[400] transition-opacity duration-500 pointer-events-none rounded-[24px]"></div>
                            )}
                            <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-md px-3 py-1.5 rounded-full flex items-center gap-1.5 border border-[#E9F4EC] shadow-sm z-[500] pointer-events-none">
                                ...
                                <span className="text-[9px] uppercase font-black tracking-widest text-[#184F35]">{t("live")}</span>
                            </div>
                        </div>
                        ...
                        <div className="bg-[#F8FBF8] rounded-[20px] p-3 border border-[#E9F4EC] flex justify-between items-center text-[10px] font-bold text-[#6C8576] uppercase tracking-wider">
                            <div className="flex items-center gap-1.5">
                                <div className="w-2.5 h-2.5 rounded-full bg-[#4CAF50]"></div> {t("optimal")}
                            </div>
                            <div className="flex items-center gap-1.5">
                                <div className="w-2.5 h-2.5 rounded-full bg-[#F9A825]"></div> {t("attention")}
                            </div>
                            <div className="flex items-center gap-1.5">
                                <div className="w-2.5 h-2.5 rounded-full bg-[#C62828] animate-pulse"></div> {t("critical")}
                            </div>
                        </div>
                    </div>
                </motion.div>
                ...
                {/* Dynamic Display */}
                <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.2 }} className="space-y-4">
                    <div className="px-1 flex justify-between items-end">
                        <h2 className="text-[17px] font-extrabold text-[#113A28]">
                            {selectedNode ? t("node_analytics") : t("active_alerts")}
                        </h2>
                        {selectedNode && (
                             <button onClick={() => setSelectedNode(null)} className="text-[11px] font-bold text-[#6C8576] hover:text-[#184F35] uppercase tracking-wider">
                                 {t("close")}
                             </button>
                        )}
                    </div>
                    ...
                    <AnimatePresence mode="wait">
                        {selectedNode ? (
                            <motion.div key="node-details" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.2 }}>
                                {(() => {
                                    ...
                                    return (
                                        <div className="bg-white rounded-[32px] p-4 shadow-[0_12px_32px_rgba(0,0,0,0.05)] border border-white">
                                            <div className="flex items-center gap-3 mb-4">
                                                <div className={cn("w-12 h-12 rounded-[16px] flex items-center justify-center shrink-0 border border-white",
                                                    isCrit ? "bg-red-50 text-red-600 shadow-[0_4px_12px_rgba(239,68,68,0.1)]" :
                                                        isWarn ? "bg-[#FFF8DF] text-[#E7A600] shadow-[0_4px_12px_rgba(231,166,0,0.1)]" :
                                                            "bg-[#F4F9F4] text-[#184F35] shadow-[0_4px_12px_rgba(24,79,53,0.1)]"
                                                )}>
                                                    <Radio size={20} className={isCrit ? "animate-pulse" : ""} />
                                                </div>
                                                <div>
                                                    <h3 className="text-[18px] font-extrabold text-[#113A28] leading-snug">{nodeData.node}</h3>
                                                    <p className={cn("text-[10px] font-black mt-0.5 uppercase tracking-wider", 
                                            ...
                                            <div className="flex gap-2 mb-4">
                                                <div className="flex-1 bg-[#F8FBF8] rounded-[20px] p-3 border border-[#E9F4EC] flex items-center gap-3">
                                                    <div className="bg-blue-50 p-2 rounded-xl text-blue-500 shrink-0"><Droplets size={16} /></div>
                                                    <div>
                                                        <div className="text-[10px] uppercase font-bold text-[#6C8576]">{t("moisture")}</div>
                                                        <div className="text-[16px] font-black text-[#113A28] leading-none mt-1">{nodeData.moisture}</div>
                                                    </div>
                                                </div>
                                                <div className="flex-1 bg-[#F8FBF8] rounded-[20px] p-3 border border-[#E9F4EC] flex items-center gap-3">
                                                    <div className="bg-orange-50 p-2 rounded-xl text-orange-500 shrink-0"><Thermometer size={16} /></div>
                                                    <div>
                                                        <div className="text-[10px] uppercase font-bold text-[#6C8576]">{t("temp")}</div>
                                                        <div className="text-[16px] font-black text-[#113A28] leading-none mt-1">{nodeData.temp}</div>
                                                    </div>
                                                </div>
                                            </div>

                                            {(isCrit || isWarn) ? (
                                                <div className={cn("rounded-[20px] p-4 mb-4 border relative overflow-hidden",
                                                    isCrit ? "bg-red-50 border-red-100" : "bg-[#FFF8DF] border-[#FFEBB3]"
                                                )}>
                                                    <div className="flex items-start gap-2 relative z-10">
                                                        <AlertTriangle size={16} className={cn("shrink-0 mt-0.5", isCrit ? "text-red-600" : "text-[#E7A600]")} />
                                                        <div>
                                                            <h5 className={cn("font-bold text-[13px] mb-1", isCrit ? "text-red-900" : "text-[#9A6E00]")}>{nodeData.issue}</h5>
                                                            <p className={cn("text-[11px] font-medium leading-relaxed", isCrit ? "text-red-800" : "text-[#B38000]")}>{nodeData.details}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            ) : (
                                                <div className="mb-4 bg-[#F4F9F4] p-4 rounded-[20px] border border-[#E9F4EC] flex items-start gap-2">
                                                    ...
                                                    <div>
                                                        <h5 className="font-bold text-[#184F35] text-[13px] mb-0.5">{t("optimal_conditions")}</h5>
                                                        <p className="text-[11px] font-medium text-[#6C8576] leading-relaxed">{t("no_intervention")}</p>
                                                    </div>
                                                </div>
                                            )}
                            ...
                            <motion.div key="alerts" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col gap-3">
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <h4 className="text-[14px] font-extrabold text-[#113A28] leading-tight flex items-center gap-1.5">
                                                <span className="w-1.5 h-1.5 rounded-full bg-red-500"></span> {t("sector_a")} - {node.node}
                                            </h4>
                                ...
                                {criticalNodes.length === 0 && (
                                     <div className="text-center p-5 bg-white border border-[#E9F4EC] shadow-sm rounded-[24px]">
                                        ...
                                        <p className="text-[11px] font-medium text-[#6C8576] mt-1">{t("no_critical_issues")}</p>
                                    </div>
                                )}
                            </motion.div>
                        )}
